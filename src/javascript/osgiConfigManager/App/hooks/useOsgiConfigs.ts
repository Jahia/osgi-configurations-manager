...
export const useOsgiConfigs = () => {
    ...
    // File-level actions (toggle, delete, mark-as-default, create, create-from-metatype, upload)
    // live in useFileActions. They were ~250 lines inline here; the unsaved-changes guarding that
    // wraps them stays in index.jsx, so this move does not touch it.
    // toggleFileStatus and handleCreateFileFromMetatype are not destructured: they were internal
    // helpers here too, never returned, and useFileActions now owns them.
    const {
        handleToggleFile,
        handleDeleteFile,
        handleMarkAsDefault,
