...
describe('OSGi Configurations Manager - Download', () => {
    ...
    it('downloads the selected file with its content and filename', () => {
        ...
        // The openOsgiFile command only waits for the NAME to appear in the header, which happens as soon as
            ...
            cy.stub(win.document, 'createElement').callsFake((tag: string) => {
                ...
                if (tag === 'a') {
                    ...
                    Object.defineProperty(el, 'download', {
                        set(v) {
                            captured.downloadName = v;
                            setter?.set?.call(this, v);
                        },
                        get() {
                            return captured.downloadName;
                        },
                        configurable: true
                    });
                }
        ...
        // A should() retries and a then() does not — so the capture is awaited rather than sampled once.
